class PaypalPage {
    ...
    get completePurchaseBtn() {
        return $("//button[contains(., 'Complete Purchase')]");
    }
    get errorMessage() { return $('.paypal-error-message'); } 


    async loginAndPay(email, password) {
        ...
        if (await cookieBanner.isDisplayed()) {
            ...
            if (await acceptBtn.isExisting()) {
                ...
                await browser.execute(() => {
                    const banner = document.querySelector('div.CookieBanner_container_wrapper');
    ...
    // Login
     ...
     async completePurchase() {
        ...
        try {
            ...
            await browser.execute((el) => el.click(), completePurchaseBtn);
        }
